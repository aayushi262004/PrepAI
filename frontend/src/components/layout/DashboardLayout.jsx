...
import { LayoutDashboard, Video, FileText, Sparkles, LogOut, Menu, ChevronRight } from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { to: '/career',    icon: Sparkles,        label: 'Career Assistant', badge: 'RAG' },
]
    ...
    <div className="flex flex-col h-full">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                {badge && (
                  <span className="text-xs px-1.5 py-0.5 rounded-full bg-violet-600/20 text-violet-300 border border-violet-600/30">
                    {badge}
                  </span>
                )}
                {isActive && !badge && <ChevronRight className="w-3 h-3 opacity-60" />}
              </>
            )}
          </NavLink>
        ))}
      ...
      <div className="p-3 border-t border-line space-y-2">
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface-300/50">
          ...
          <button onClick={handleLogout} className="text-ink-faint hover:text-red-400 transition-colors p-1 rounded-lg hover:bg-red-500/10">
...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-surface overflow-hidden">
      <aside className="hidden lg:flex w-56 xl:w-60 flex-shrink-0 bg-surface-100 border-r border-line flex-col">
        <SidebarContent />
      </aside>
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            <motion.aside initial={{ x: -260 }} animate={{ x: 0 }} exit={{ x: -260 }}
              ...
              className="fixed left-0 top-0 h-full w-60 bg-surface-100 border-r border-line z-40 lg:hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="lg:hidden flex items-center gap-3 px-4 py-3 border-b border-line bg-surface-100">
          <button onClick={() => setMobileOpen(true)} className="text-ink-muted hover:text-ink p-1">
            <Menu className="w-5 h-5" />
          </button>
          <span className="font-display font-bold text-ink">PrepAI</span>
        </header>

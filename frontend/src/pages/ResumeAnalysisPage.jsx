...
function ScoreGauge({ score }) {
  ...
  return (
    <div className="flex flex-col items-center py-6">
      ...
      <div className="text-ink-faint text-xl mt-1">/ 100</div>
  ...
  return (
    <div className="glass-card mb-5">
      ...
      <div className="flex items-center gap-5">
        <div className={`text-4xl font-display font-bold ${color} tabular-nums flex-shrink-0`}>{score}<span className="text-lg text-ink-faint">/100</span></div>
  ...
  return (
    <div>
      <div className="flex justify-between text-sm mb-1">
        ...
        <span className="text-ink-muted font-mono">{score}/{max}</span>
      </div>
...
function FileDropZone({ onFile, file }) {
  ...
  return (
    <label onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={handleDrop}
      className={`flex flex-col items-center justify-center gap-3 p-8 rounded-xl border-2 border-dashed cursor-pointer transition-all ${dragging ? 'border-brand-500 bg-brand-600/10' : file ? 'border-emerald-600/40 bg-emerald-600/5' : 'border-line-strong bg-surface-200 hover:border-brand-600/40'}`}>
      <input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={e => onFile(e.target.files[0])} />
      {file ? (<><FileText className="w-8 h-8 text-emerald-400" /><p className="text-sm font-medium text-emerald-300">{file.name}</p><p className="text-xs text-ink-faint">{(file.size / 1024).toFixed(1)} KB · Click to change</p></>) : (<><Upload className="w-8 h-8 text-ink-faint" /><p className="text-sm font-medium text-ink-muted">Drop your resume here</p><p className="text-xs text-ink-faint mt-1">PDF or DOCX · Max 5MB</p></>)}
    </label>
  )
}
  ...
  if (result) {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl">
        ...
        <p className="page-subheader">Analyzed: <span className="text-ink-muted">{result.fileName}</span></p>

        {/* Score */}
        <div className="glass-card mb-5">
          <ScoreGauge score={result.score} />
          {result.summary && <p className="text-center text-ink-muted text-sm mt-2 max-w-lg mx-auto leading-relaxed">{result.summary}</p>}
        </div>
        ...
        {result.strengths?.length > 0 && (
          <div className="glass-card mb-5">
            ...
            <ul className="space-y-2">{result.strengths.map((s, i) => <li key={i} className="flex items-start gap-2 text-sm text-ink-muted"><CheckCircle className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />{s}</li>)}</ul>
          </div>
        )}
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-5">
            {result.experienceGap && (
              <div className="glass-card">
                ...
                <p className="text-sm text-ink-muted leading-relaxed">{result.experienceGap}</p>
              </div>
            )}
            {result.toneFeedback && (
              <div className="glass-card">
                <h3 className="section-title">Tone & Writing</h3>
                <p className="text-sm text-ink-muted leading-relaxed">{result.toneFeedback}</p>
              </div>
            )}
          </div>
        )}
        ...
        {result.recommendations?.length > 0 && (
          <div className="glass-card">
            ...
            <ul className="space-y-3">{result.recommendations.map((r, i) => <li key={i} className="flex items-start gap-3 text-sm text-ink-muted"><span className="flex-shrink-0 w-5 h-5 rounded-full bg-amber-600/20 text-amber-300 text-xs flex items-center justify-center font-medium mt-0.5">{i + 1}</span>{r}</li>)}</ul>
          </div>
        )}
      </motion.div>
    )
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl">
      ...
      <form onSubmit={handleAnalyze} className="space-y-5">
        ...
        <div>
          <label className="label">Job Description <span className="text-ink-faint">(optional — recommended for skill-gap accuracy)</span></label>
          <textarea
      ...
      {/* How it works */}
      <div className="mt-8 p-4 rounded-xl bg-surface-200 border border-line">
        <p className="text-xs font-medium text-ink-faint uppercase tracking-wider mb-3">How it works</p>
